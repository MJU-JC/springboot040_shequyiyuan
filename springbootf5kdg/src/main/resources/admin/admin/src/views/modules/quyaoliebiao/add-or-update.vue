      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="诊断结果" prop="zhenduanjieguo">
          <el-input v-model="ruleForm.zhenduanjieguo" 
              placeholder="诊断结果" clearable  :readonly="ro.zhenduanjieguo"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="诊断结果" prop="zhenduanjieguo">
              <el-input v-model="ruleForm.zhenduanjieguo" 
                placeholder="诊断结果" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="开药列表" prop="kaiyaoliebiao">
          <el-input v-model="ruleForm.kaiyaoliebiao" 
              placeholder="开药列表" clearable  :readonly="ro.kaiyaoliebiao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="开药列表" prop="kaiyaoliebiao">
              <el-input v-model="ruleForm.kaiyaoliebiao" 
                placeholder="开药列表" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="取药窗口" prop="quyaochuangkou">
          <el-select v-model="ruleForm.quyaochuangkou" placeholder="请选择取药窗口">
            <el-option
                v-for="(item,index) in quyaochuangkouOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="取药窗口" prop="quyaochuangkou">
	      <el-input v-model="ruleForm.quyaochuangkou"
                placeholder="取药窗口" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="取药状态" prop="quyaozhuangtai">
          <el-select v-model="ruleForm.quyaozhuangtai" placeholder="请选择取药状态">
            <el-option
                v-for="(item,index) in quyaozhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="取药状态" prop="quyaozhuangtai">
	      <el-input v-model="ruleForm.quyaozhuangtai"
                placeholder="取药状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  data() {
	...
	kaiyaoliebiao : false,
	quyaochuangkou : false,
	quyaozhuangtai : false,
	userid : false,
      },
      ruleForm: {
        huanzhezhanghao: '',
        ...
        userid: '',
          ...
          quyaochuangkouOptions: [],
          quyaozhuangtaiOptions: [],
      rules: {
          huanzhezhanghao: [
          ],
          huanzhexingming: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kaiyaoliebiao = true;
          ...
          if(o=='quyaochuangkou'){
            this.ruleForm.quyaochuangkou = obj[o];
	    this.ro.quyaochuangkou = true;
            continue;
          }
          if(o=='quyaozhuangtai'){
            this.ruleForm.quyaozhuangtai = obj[o];
	    this.ro.quyaozhuangtai = true;
            continue;
          }
          if(o=='userid'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.huanzhezhanghao!=''&&json.huanzhezhanghao){
              		this.ruleForm.huanzhezhanghao = json.huanzhezhanghao
		}
		if(json.huanzhexingming!=''&&json.huanzhexingming){
          ...
          this.$message.error(data.msg);
            ...
            this.quyaochuangkouOptions = "一号窗口,二号窗口,三号窗口".split(',')
            this.quyaozhuangtaiOptions = "未取药,已取药".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `quyaoliebiao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `quyaoliebiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.quyaoliebiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
